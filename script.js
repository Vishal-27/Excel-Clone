...
function changeHeader(ele)
{
    ...
    // for multiway in font family
    $(".font-family-selector").val(cellInfo["font-family"]);

    // for multiway in font size

    $(".font-size-selector").val(cellInfo["font-size"]);

// to reflect font family in the header
   ...
   //  left scroller for column and top for row
...
// for bold , italize and underline the text 
  ...
  $(".input-cell.selected").each(function(){
      ...
      let [rowId ,colId]=getRowCol(this);
        ...
        else
        {
          ...
          // after updating it become same to column
...
// for color picker and text picker
...
// for changing the background color of  a cell
...
// for changing the text color of a cell
...
// for deleting the entry of a sheet
...
function loadSheet()
{
    ...
    for(let i of Object.keys(sheetInfo))
    {
        for(let j of Object.keys(sheetInfo[i]))
        {
         ...
         $(`#row-${i}-col-${j}`).css("text-decoration",cellInfo["tex"]);
...
// add event listener on added sheet
 addSheetEvents();
});
...
function addSheetEvents()
{
    ...
    $(".sheet-tab").contextmenu(function(e)
    {
      ...
      selectSheet(this);
      // only append when we work for first time on a sheet
   ...
   $(".sheet-rename").click(function()
   {
       ...
       <h4 class="modal-title">Rename to :</h4>
...
// we create a new sheet array and copy all the data of the previous array

let newCellData={}
for(key in cellData)
{
    // key represent the sheet
...
cellData = newCellData;
   ...
   $(".sheet-delete").click(function(){
    ...
    {
        alert("Only 1 sheet present therfore deletion not possible");
    }
})
